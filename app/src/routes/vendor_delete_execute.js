...
let LOG_FILE_NAME = "vendor_delete_execute";
...
router.post("/", function (req, res, next) {
  ...
  if (req.body.current_vendor_id === undefined || req.body.current_vendor_id === null) {
  ...
  if (Number.isFinite(req.body.current_vendor_id)) {
    req.session.body = body;
    req.session.error = common_webserver_util_func.get_parameter_error_message();
    let back_url = req.session.backURL || '/dashboard';
    res.redirect(back_url);
  ...
  let parameter = { "vendor_id": req.body.current_vendor_id };
  ...
  common_webserver_util_func.execute_delete_vendor_api(parameter, apikey, secretkey,
    (error, response, body) => {
      if (error !== null) {
