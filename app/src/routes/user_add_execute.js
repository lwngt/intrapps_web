...
let LOG_FILE_NAME = "user_add_execute";
...
router.post("/", upload.none(), function (req, res, next) {
  ...
  if (req.body.user_manage_projects !== undefined && req.body.user_manage_projects !== null) {
    // ok is user_manage_projects is null
  ...
  if (Array.isArray(user_manage_projects)) {
    user_manage_projects.forEach(project => {
      if (project.checked === "true") {
        user_manage_projects_array.push(project.id);
      }
    });
  } else {
    if (user_manage_projects !== null
      ...
      if (user_manage_projects.checked === "true") {
        user_manage_projects_array.push(user_manage_projects.id);
      }
    }
  }
  ...
  common_webserver_util_func.execute_user_add_api(parameter, apikey, secretkey,
    (error, response, body) => {
      if (error !== null) {
